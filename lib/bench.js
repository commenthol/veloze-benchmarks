...
const doBench = async (opts, handler) => {
  let { url = '' } = opts
  if (handler.endsWith('-big-json') || handler.endsWith('-with-middlewares')) {
    url = ''
  }

  const spinner = ora(`Started ${handler}`).start()
  const forked = fork(join(__dirname, '..', 'benchmarks', handler + '.js'))
  try {
    spinner.color = 'magenta'
    spinner.text = `Warming ${handler}`
    await fire({ ...opts, url, duration: 2 }, handler, false)
  } catch (error) {
  ...
  try {
    await fire({ ...opts, url }, handler, true)
